            ...
            <div class="overflow-y-auto max-h-[400px] px-4 custom-scrollbar">
                <table class="min-w-full text-sm text-emerald-900">
                    ...
                    <tbody class="divide-y divide-white/60">
                        @forelse ($data as $item)
                            <tr>
                                ...
                                <td class="p-3 text-center">
                                    ...
                                    @else
                                        <span class="bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-xs">Belum Bayar</span>
                                    @endif
                                </td>
                                <td class="p-3 text-center">
                                    @if(!$item->file_kk)
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="p-3 text-center text-emerald-900">Tidak ada pendaftaran yang sudah disetujui.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                if (data.status === 'success') {
                    ...
                    window.snap.pay(data.snap_token, {
                        onSuccess: function(result) {
                            window.location.href = '{{ route("daftarUlang.index") }}?payment=success';
                        },
                        onPending: function(result) {
                            window.location.href = '{{ route("daftarUlang.index") }}?payment=pending';
                        },
                        onError: function(result) {
                            window.location.href = '{{ route("daftarUlang.index") }}?payment=error';
                        },
                        onClose: function() {

    ...
    <section class="font-sans min-h-screen bg-cover bg-center relative"
        ...
        @if (session('success'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" data-success-alert
                class="fixed bottom-5 right-5 z-[9999] w-full max-w-sm bg-green-600 text-white rounded-xl p-4 shadow-lg flex items-start gap-3 animate-slide-up transition-all duration-500 ease-in-out">
        ...
        @if ($errors->any())
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" data-error-alert
                class="fixed bottom-5 right-5 z-[9999] w-full max-w-sm bg-red-600 text-white rounded-xl p-4 shadow-lg flex items-start gap-3 animate-slide-up transition-all duration-500 ease-in-out">
                <div class="flex-shrink-0 w-10 h-10 bg-white rounded-full flex items-center justify-center">
                    <img src="{{ asset('assets/ppmq.png') }}" alt="Logo" class="h-6 w-6">
                </div>
                <div class="flex-grow">
        ...
        <div x-data="{ showTambahPendaftaran: false }"
            class="flex flex-col items-center justify-center min-h-screen pt-28 px-8 relative z-10">
                ...
                <div class="overflow-y-auto max-h-[300px] min-h-[300px] px-4 custom-scrollbar">
                    <table class="min-w-full text-sm text-emerald-900">
                        ...
                        <tbody class="divide-y divide-white/60">
                            @forelse ($data as $item)
                                ...
                                <tr class="hover:bg-white/10 transition">
                                    <td class="p-3 text-center">
                                        <div
                                            class="bg-emerald-900/20 backdrop-blur-sm px-3 py-1 rounded-md text-sm flex items-center justify-center">
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="p-3 text-center text-gray-400">Tidak ada pendaftaran yang
                                        sedang diproses.</td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div @click.outside="showTambahPendaftaran = false"
                    ...
                    <form action="{{ route('pendaftaran.simpan') }}" method="POST" class="space-y-4 min-w-[400px]">
                        ...
                        <div class="flex justify-end gap-2">
                            <button type="button" @click="showTambahPendaftaran = false"
            ...
            <div class="w-full max-w-5xl mt-4 mx-auto">
                ...
                @else
                    <button disabled class="px-6 py-2 bg-gray-500 text-white rounded-lg cursor-not-allowed">
